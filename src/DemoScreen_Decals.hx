...
var decals:DecaleNode;
	...
	mesh = MeshCreator.loadStaticB3DMesh("data/models/castel.b3d", "data/models/textures/");
	mesh.Scale(0.2, 0.2, 0.2);

	scene.addSceneNode(mesh);
	
	point = scene.addSceneNode(MeshCreator.createCube());
	point.setTexture(Gdx.Instance().getTexture("data/marble.jpg",true,true, false));
	...
	font.caption = Gdx.Instance().status+"\n\n\n B3D Static mesh + Decals , right click on mesh" ;
